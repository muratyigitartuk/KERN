...
class RoutineService:
    ...
    def run(self, name: str) -> ToolResult:
        ...
        return ToolResult(
            ...
            spoken_text="I do not have that routine configured locally.",
    ...
    def _morning(self) -> ToolResult:
        ...
        if brief.music_suggestion:
            pieces.append(f"Suggested start: {brief.music_suggestion}.")
        spoken = " ".join(pieces[:4])
        return ToolResult(
            success=True,
            status="observed",
    ...
    def _focus(self) -> ToolResult:
        ...
        spoken = f"Focus routine is live. Center on {top_task}. I set a 50 minute timer."
        return ToolResult(
            success=True,
            status="observed",
    ...
    def _shutdown(self) -> ToolResult:
        ...
        count = len(remaining)
        spoken = (
            f"Shutdown routine ready. You have {count} tasks left."
            if count
            else "Shutdown routine ready. No remaining tasks are open."
        )
        return ToolResult(
            success=True,
            status="observed",
